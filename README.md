...
1. **全局键盘监听**：使用 `NSEvent.addGlobalMonitorForEvents` 实现系统级键盘监听
2. **权限管理**：优雅处理辅助功能权限请求和检查
3. **实时数据更新**：使用 `@Published` 和 Combine 实现响应式数据流
4. **高效数据存储**：使用时间戳数组存储，支持快速时间范围查询
5. **自动数据清理**：自动清理7天以上的旧数据，避免存储空间浪费
6. **美观的UI设计**：使用SwiftUI构建现代化界面，支持实时更新
